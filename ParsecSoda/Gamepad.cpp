...
bool Gamepad::setState(ParsecGamepadStateMessage state)
{
	...
	{
		_currentState.Gamepad.wButtons = state.buttons;
		_currentState.Gamepad.bLeftTrigger = state.leftTrigger;
		_currentState.Gamepad.bRightTrigger = state.rightTrigger;
		_currentState.Gamepad.sThumbLX = state.thumbLX;
		_currentState.Gamepad.sThumbLY = state.thumbLY;
		_currentState.Gamepad.sThumbRX = state.thumbRX;
		_currentState.Gamepad.sThumbRY = state.thumbRY;

		if (mirror)
		{
			Bitwise::setValue(&_currentState.Gamepad.wButtons, XUSB_GAMEPAD_DPAD_LEFT, state.thumbLX < -GAMEPAD_DEADZONE);
			Bitwise::setValue(&_currentState.Gamepad.wButtons, XUSB_GAMEPAD_DPAD_RIGHT, state.thumbLX > GAMEPAD_DEADZONE);
			Bitwise::setValue(&_currentState.Gamepad.wButtons, XUSB_GAMEPAD_DPAD_UP, state.thumbLY > GAMEPAD_DEADZONE);
			Bitwise::setValue(&_currentState.Gamepad.wButtons, XUSB_GAMEPAD_DPAD_DOWN, state.thumbLY < -GAMEPAD_DEADZONE);
		}
...
bool Gamepad::setState(ParsecKeyboardMessage key)
{
	if (_isAlive && _isConnected && _client != nullptr)
	{
		...
		case (int)KEY_TO_GAMEPAD2::LEFT:
			_currentState.Gamepad.sThumbLX = (key.pressed ? GAMEPAD_STICK_MIN : 0);
			if (mirror)
			{
				Bitwise::setValue(&_currentState.Gamepad.wButtons, XUSB_GAMEPAD_DPAD_LEFT, key.pressed);
			}
			break;
		case (int)KEY_TO_GAMEPAD::RIGHT:
		case (int)KEY_TO_GAMEPAD2::RIGHT:
			_currentState.Gamepad.sThumbLX = (key.pressed ? GAMEPAD_STICK_MAX : 0);
			if (mirror)
			{
				Bitwise::setValue(&_currentState.Gamepad.wButtons, XUSB_GAMEPAD_DPAD_RIGHT, key.pressed);
			}
			break;
		case (int)KEY_TO_GAMEPAD::UP:
		case (int)KEY_TO_GAMEPAD2::UP:
			_currentState.Gamepad.sThumbLY = (key.pressed ? GAMEPAD_STICK_MAX : 0);
			if (mirror)
			{
				Bitwise::setValue(&_currentState.Gamepad.wButtons, XUSB_GAMEPAD_DPAD_UP, key.pressed);
			}
			break;
		case (int)KEY_TO_GAMEPAD::DOWN:
		case (int)KEY_TO_GAMEPAD2::DOWN:
			_currentState.Gamepad.sThumbLY = (key.pressed ? GAMEPAD_STICK_MIN : 0);
			if (mirror)
			{
				Bitwise::setValue(&_currentState.Gamepad.wButtons, XUSB_GAMEPAD_DPAD_DOWN, key.pressed);
			}
			break;
		...
		case (int)KEY_TO_GAMEPAD2::A:
			Bitwise::setValue(&_currentState.Gamepad.wButtons, XUSB_GAMEPAD_A, key.pressed);
			break;
		case (int)KEY_TO_GAMEPAD::B:
		case (int)KEY_TO_GAMEPAD2::B:
			Bitwise::setValue(&_currentState.Gamepad.wButtons, XUSB_GAMEPAD_B, key.pressed);
			break;
		case (int)KEY_TO_GAMEPAD::X:
		case (int)KEY_TO_GAMEPAD2::X:
			Bitwise::setValue(&_currentState.Gamepad.wButtons, XUSB_GAMEPAD_X, key.pressed);
			break;
		case (int)KEY_TO_GAMEPAD::Y:
		case (int)KEY_TO_GAMEPAD2::Y:
			Bitwise::setValue(&_currentState.Gamepad.wButtons, XUSB_GAMEPAD_Y, key.pressed);
			break;

			// Center
		case (int)KEY_TO_GAMEPAD::BACK:
		case (int)KEY_TO_GAMEPAD2::BACK:
			Bitwise::setValue(&_currentState.Gamepad.wButtons, XUSB_GAMEPAD_BACK, key.pressed);
			break;
		case (int)KEY_TO_GAMEPAD::START:
		case (int)KEY_TO_GAMEPAD2::START:
			Bitwise::setValue(&_currentState.Gamepad.wButtons, XUSB_GAMEPAD_START, key.pressed);
			break;

			// Shoulders
		case (int)KEY_TO_GAMEPAD::LB:
		case (int)KEY_TO_GAMEPAD2::LB:
			Bitwise::setValue(&_currentState.Gamepad.wButtons, XUSB_GAMEPAD_LEFT_SHOULDER, key.pressed);
			break;
		case (int)KEY_TO_GAMEPAD::RB:
		case (int)KEY_TO_GAMEPAD2::RB:
			Bitwise::setValue(&_currentState.Gamepad.wButtons, XUSB_GAMEPAD_RIGHT_SHOULDER, key.pressed);
			break;

			// Triggers
		case (int)KEY_TO_GAMEPAD::LT:
		case (int)KEY_TO_GAMEPAD2::LT:
			_currentState.Gamepad.bLeftTrigger = (key.pressed ? GAMEPAD_STICK_MAX : GAMEPAD_STICK_MIN);
			break;
		case (int)KEY_TO_GAMEPAD::RT:
		case (int)KEY_TO_GAMEPAD2::RT:
			_currentState.Gamepad.bRightTrigger = (key.pressed ? GAMEPAD_STICK_MAX : GAMEPAD_STICK_MIN);
			break;

			// Thumbs
		case (int)KEY_TO_GAMEPAD::LTHUMB:
		case (int)KEY_TO_GAMEPAD2::LTHUMB:
			Bitwise::setValue(&_currentState.Gamepad.wButtons, XUSB_GAMEPAD_LEFT_THUMB, key.pressed);
			break;
		case (int)KEY_TO_GAMEPAD::RTHUMB:
		case (int)KEY_TO_GAMEPAD2::RTHUMB:
			Bitwise::setValue(&_currentState.Gamepad.wButtons, XUSB_GAMEPAD_RIGHT_THUMB, key.pressed);
			break;
...
bool Gamepad::setState(ParsecGamepadButtonMessage button)
{
	if (_isAlive && _isConnected && _client != nullptr)
	{
		...
		if (isOk)
		{
			Bitwise::setValue(&_currentState.Gamepad.wButtons, buttonCode, button.pressed);
			vigem_target_x360_update(_client, pad, *reinterpret_cast<XUSB_REPORT*>(&_currentState.Gamepad));
...
bool Gamepad::setState(ParsecGamepadAxisMessage axis)
{
	if (_isAlive && _isConnected && _client != nullptr)
	{
		...
		case GAMEPAD_AXIS_LX:
			_currentState.Gamepad.sThumbLX = axis.value;
			if (mirror)
			{
				Bitwise::setValue(&_currentState.Gamepad.wButtons, XUSB_GAMEPAD_DPAD_LEFT, axis.value < -GAMEPAD_DEADZONE);
				Bitwise::setValue(&_currentState.Gamepad.wButtons, XUSB_GAMEPAD_DPAD_RIGHT, axis.value > GAMEPAD_DEADZONE);
			}
			break;
		case GAMEPAD_AXIS_LY:
			_currentState.Gamepad.sThumbLY = -axis.value;
			if (mirror)
			{
				Bitwise::setValue(&_currentState.Gamepad.wButtons, XUSB_GAMEPAD_DPAD_UP, axis.value < -GAMEPAD_DEADZONE);
				Bitwise::setValue(&_currentState.Gamepad.wButtons, XUSB_GAMEPAD_DPAD_DOWN, axis.value > GAMEPAD_DEADZONE);
			}
			break;
		case GAMEPAD_AXIS_RX:
			_currentState.Gamepad.sThumbRX = axis.value;
			break;
		case GAMEPAD_AXIS_RY:
			_currentState.Gamepad.sThumbRY = -axis.value;
			break;
		case GAMEPAD_AXIS_TRIGGERL:
			_currentState.Gamepad.bLeftTrigger = axis.value;
			break;
		case GAMEPAD_AXIS_TRIGGERR:
			_currentState.Gamepad.bRightTrigger = axis.value;
			break;
		default:
			isOk = false;
			break;
